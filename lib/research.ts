...
==================================================
8단계. Auto Publisher 전달 규격 — 껍데기는 고정, 본문 사고방식은 자유
==================================================

아래 최종 출력은 사람에게 보여주기 위한 글 템플릿이 아니라 Auto Publisher와 데이터를 주고받기 위한 “전달 규격”이다.
따라서 # 1~4 상위 섹션명, 게시용 본문 시작/끝 주석, Rank Math 필드명은 매번 정확히 동일하게 유지한다.
...
최종 원고를 쓰기 직전에 “이 글만의 판단 흐름”을 한 문장으로 내부 점검해라. 다른 생활정보 글에도 제목만 바꿔 그대로 적용할 수 있는 흐름이라면 다시 설계한다.

==================================================
최종 출력 형식 — Auto Publisher 표준 Markdown
==================================================

아래 상위 섹션명과 순서를 그대로 사용하고, 별도 잡담을 앞뒤에 넣지 마라.
[게시 보류]인 경우에는 완성 원고를 만들지 말고 “# 2. 게시 전 확인 자료”만 작성해 보류 이유와 더 강한 주제 변형 1개를 제안한다.
[게시 진행] 또는 [보완 후 게시]인 경우에는 다음 규격을 정확히 사용한다.
...
## 이 글만의 판단 흐름
이 주제에서만 자연스러운 문제 해결 순서를 한 문장으로 작성
...
중요한 전달 규칙:
1. “# 1. 게시용 본문”, “# 2. 게시 전 확인 자료”, “# 3. 대표 이미지 생성 프롬프트”, “# 4. Rank Math SEO 패키지” 이름을 바꾸지 않는다.
2. 게시용 본문은 반드시 <!-- 게시용 본문 시작 --> / <!-- 게시용 본문 끝 --> 사이에 둔다.
3. 게시용 본문의 첫 번째 H1은 Auto Publisher가 게시 제목으로 추출하며 실제 WordPress 본문에서는 제거된다.
4. Rank Math 각 값은 해당 H2 바로 다음의 첫 번째 유효한 줄에 한 줄 값으로 출력한다.
5. 위 전달 규격을 고정한다고 해서 실제 글의 구조를 규격화하지 않는다. 고정되는 것은 데이터 포맷뿐이다.
