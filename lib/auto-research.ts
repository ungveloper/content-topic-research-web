 ...
 * NAVER 지식iN/카페의 실제 질문에서 최종 주제를 발견하기 위한 출발점으로만 사용합니다.
 */
...
const BASE_SCOPES: DiscoveryScope[] = [
  ...
  },
  {
    ...
    queries: ["생활 행정 신청", "온라인 민원 불편", "생활 신고 방법", "공공서비스 이용 문제", "이사 행정 준비"],
    audience: "신고·신청·배출·공공서비스처럼 생활 행정 절차에서 필요한 준비와 완료 조건을 확인하려는 사용자",
    evergreen: 4,
    contentMode: "research-verification",
    uniqueOutput: "공식 신청 페이지를 기준으로 준비물·진행 단계·예외·완료 확인을 다시 배열한 실행 체크리스트",
    verificationPlan: "정부·지자체·공공기관 원문을 기준으로 절차와 날짜를 검증하고 뉴스는 변경 감지용으로만 사용한다.",
  },
  {
...
const SEASONAL_QUERIES: Record<number, string[]> = {
  ...
  2: ["이사철 생활 준비", "겨울 끝 집 정리", "봄 준비 생활", "생활 행정 이사"],
