...
[research-verification 통과 시 최종 출력 형식 — 기존 번호 선택 후 아웃풋과 동일하게 유지]
research-verification으로 검증이 충분한 경우에는 아래 4개 상위 섹션만 출력한다. “게시 전 확인 자료”, 별도 검증 보고서, 내부 판단 메모, 추천 후보, 순위, 앱 import JSON은 출력하지 않는다. # 4는 WordPress 본문에 게시하는 영역이 아니라, 이 글의 출발점이 된 네이버 지식iN 질문에 남길 수 있는 별도 답변 추천 영역이다.
...
이 섹션은 WordPress 게시용 본문에 포함하지 않는다. 이번 Focused Research에서 사용자가 직접 입력한 지식iN 원문 질문을 그대로 대표 질문으로 사용한다.

## 원문 질문
- 제목: ${context.focusedSourceQuestion.title}
- 원문 URL: ${context.focusedSourceQuestion.url}
- 질문 내용: 가능하면 위 원문 URL을 직접 열어 질문자가 적은 상황과 조건을 확인한다. 질문이 짧으면 필요한 범위에서 핵심 원문을 제시할 수 있지만, 긴 질문은 통째로 복제하지 말고 의미를 바꾸지 않는 짧은 핵심 요약으로 적는다. 원문을 열 수 없다면 수집된 원문 설명 범위만 사용하고 없는 세부사항을 만들지 않는다.
